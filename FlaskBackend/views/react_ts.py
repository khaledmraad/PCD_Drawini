...
def generate_code_and_return():
    source_path = './static/react_app_typescript'
    output_zip = BytesIO()
...
def download_code(code):
    ...
    return send_file(
        zip_bytes,
        as_attachment=True,
        download_name='react_ts_app.zip'
    )  
...
def modify_zip_content(react_zip, modified_content):
    ...
    try:
        ...
        widget_container_path = os.path.join(temp_dir, 'src', 'components', 'WidgetContainer.tsx')
        if os.path.exists(widget_container_path):
            with open(widget_container_path, 'w') as f:
                f.write(modified_content)
        else:
            print("WidgetContainer.tsx not found:", widget_container_path)
